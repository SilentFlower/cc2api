...
- `auto`：推荐的保守修复策略，先稳定化 tools / skills / deferred tools / 并行 tool blocks 顺序，再接管 4 个断点预算，优先在 text 边界放置 message 断点，不主动选择 `assistant tool_use`
- `rolling`：更积极的滚动断点对照策略，同样稳定化 prefix，但会尽量用满 4 个断点；它也不会落到 `assistant tool_use`，但会在窗口回退时允许使用 `user tool_result`

历史配置值 `stable` / `anchored` 会兼容解析为 `auto`。

`auto` / `rolling` 会清理请求根级、`system`、`tools` 和 `messages` 内已有的
