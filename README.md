...
- `rolling`：并行 tool 长会话策略，先稳定化 tools / skills / deferred tools / 并行 tool blocks 顺序，再接管 4 个断点预算并按 10 个顶层 message content block 步距在尾部滚动补 message 断点

`rolling` 会清理请求根级、`system`、`tools` 和 `messages` 内已有的
...
`cache_control`，但会计入 10-block 步距。
