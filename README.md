...
- `allowed_claude_code_versions`：只作用于 `claude-code/` / `claude-cli/` UA，默认 `2.1.89-2.1.156`
- `allowed_user_agents`：只作用于非 Claude Code / CLI UA，默认允许 `AI-Hub-Monitor*` 和 `python-httpx*`
- 版本规则支持精确版本、通配和闭区间，例如 `2.1.156`、`2.1.*`、`2.1.89-2.1.156`
...
- `stable`：旧策略，清理 message 断点后按最后一条 message 和倒数第二个 user turn 重打
- `rolling`：无状态并行 tool 长会话策略，先稳定化 tools / skills / deferred tools / 并行 tool blocks 顺序，再接管 4 个断点预算并按 19 个顶层 message content block 步距在尾部滚动补 message 断点
- `anchored`：会话锚定滚动策略，在 `rolling` 基础上按 Claude Code session 记住上一轮已发往上游的 message 断点，下一轮以最新旧断点恢复缓存读，再按 19-block 步距桥接到当前尾部

`rolling` / `anchored` 会清理请求根级、`system`、`tools` 和 `messages` 内已有的
...
为避免并行 tool 场景下 prefix hash 在进入 messages 之前就变化，这两个模式还会将
...
`cache_control`，但会计入 19-block 步距。`anchored` 的断点记忆是单进程内存
状态，适合当前单容器部署；多副本部署时需要会话粘滞或持久化状态才能保证每轮都命中同一份记忆。
